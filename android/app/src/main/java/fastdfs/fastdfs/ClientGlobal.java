...
import fastdfs.common.IniFileReader;
import fastdfs.common.MyException;
...
public class ClientGlobal {
  ...
  public static void init(String conf_filename) throws IOException, MyException {
   ...
   // iniReader = new IniFileReader(conf_filename);
    ...
    g_connect_timeout = 2;//iniReader.getIntValue("connect_timeout", DEFAULT_CONNECT_TIMEOUT);
    if (g_connect_timeout < 0) {
      g_connect_timeout = DEFAULT_CONNECT_TIMEOUT;
    }
    g_connect_timeout *= 1000; //millisecond

    g_network_timeout = 30;//iniReader.getIntValue("network_timeout", DEFAULT_NETWORK_TIMEOUT);
    if (g_network_timeout < 0) {
      g_network_timeout = DEFAULT_NETWORK_TIMEOUT;
    }
    g_network_timeout *= 1000; //millisecond

    g_charset = "UTF-8";//iniReader.getStrValue("charset");
    if (g_charset == null || g_charset.length() == 0) {
      g_charset = "ISO8859-1";
    }

    String dfsUrl = SystemConfigSp.instance().getStrConfig(SystemConfigSp.SysCfgDimension.FASTDFSSERVER);

    if(TextUtils.isEmpty(dfsUrl))
      dfsUrl = UrlConstant.FASTDFS_SERVER;

    szTrackerServers = new String[]{dfsUrl};//iniReader.getValues("tracker_server");//"192.168.88.209:21222"
    ...
    g_tracker_http_port = 8080;//iniReader.getIntValue("http.tracker_http_port", 80);
    g_anti_steal_token = false;//iniReader.getBoolValue("http.anti_steal_token", false);
    if (g_anti_steal_token) {
      g_secret_key = "FastDFS1234567890";//iniReader.getStrValue("http.secret_key");
    }
    g_connection_pool_enabled = true;//iniReader.getBoolValue("connection_pool.enabled", DEFAULT_CONNECTION_POOL_ENABLED);
    g_connection_pool_max_count_per_entry = 500;//iniReader.getIntValue("connection_pool.max_count_per_entry", DEFAULT_CONNECTION_POOL_MAX_COUNT_PER_ENTRY);
    g_connection_pool_max_idle_time = 3600;//iniReader.getIntValue("connection_pool.max_idle_time", DEFAULT_CONNECTION_POOL_MAX_IDLE_TIME);
    ...
    g_connection_pool_max_wait_time_in_ms = 1000;//iniReader.getIntValue("connection_pool.max_wait_time_in_ms", DEFAULT_CONNECTION_POOL_MAX_WAIT_TIME_IN_MS);
