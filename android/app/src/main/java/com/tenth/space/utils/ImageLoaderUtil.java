...
class FastdfsImageDownloader extends BaseImageDownloader {//wystan add for fastdfs 210121

    public FastdfsImageDownloader(Context context) {
        super(context);
    }

    public InputStream getStream(String imageUri, Object extra) throws IOException {
        if(imageUri.contains(FdfsUtil.FDFS_PROTOL)){
            String ret= new FdfsUtil(context.getFilesDir().getAbsolutePath()+"/fastdfs/").downloadSingle(imageUri);
            if(null != ret){
                imageUri = "file://" + ret;
            }
        }
...
//                    .denyCacheImageMultipleSizesInMemory()
                    ...
                    .imageDownloader(new FastdfsImageDownloader(context))
