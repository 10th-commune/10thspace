...
public class GroupManagerAdapter extends BaseAdapter {
	...
	public void updateGroupData() {
        ...
        for (String memId :((GroupEntity)peerEntity).getlistGroupMemberIds()) {
    ...
    public void updateGroupData(PeerEntity peerEntity){
        this.peerEntity = peerEntity;
        setData();
    }
    ...
    private void setGroupData(GroupEntity entity){
        ...
        if(null != entity.getUserEntityList()) {
            for (UserEntity user : entity.getUserEntityList()) {
                if (ownerId.equals(user.getPeerId())) {
                    // 群主放在第一个
        ...
        switch (entity.getGroupType()){
            ...
            break;
            case DBConstant.GROUP_TYPE_ACTIVE:{
                if(loginId.equals(entity.getCreatorId())){
                    // 展示加减
                    showMinusTag = true;
                    showPlusTag = true;
	...
	public void removeById(String contactId) {
        for (UserEntity contact : memberList) {
            if (contact.getPeerId().equals(contactId)) {
		...
		if (position >= 0 && memberList.size() > position) {
			...
			if (holder.imageView != null) {
				holder.imageView.setOnClickListener( new View.OnClickListener() {
					...
					public void onClick(View v) {
						IMUIHelper.openUserProfileActivity(context, userEntity.getPeerId(),userEntity.getRelation());
					}
				});
			}
            if(groupCreatorId !="" && groupCreatorId .equals(userEntity.getPeerId())){
                holder.role.setVisibility(View.VISIBLE);
            }

            if (removeState && !userEntity.getPeerId().equals(groupCreatorId)) {
            ...
            holder.imageView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                        ...
                        IMUIHelper.openGroupMemberSelectActivity(context,peerEntity.getSessionKey(),0);
                    }
            });
            holder.deleteImg.setVisibility(View.INVISIBLE);
			...
			if (contactEntity != null) {
				...
				holder.deleteImg.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
                        ...
                        String userId = holder.contactEntity.getPeerId();
                        removeById(userId);
