...
public class IMUnreadMsgManager extends IMManager {
    ...
    public void getUnreadSysMsgRequest(int unreadcnt, IMSystem.SysMsgType type) {
        //获取未读系统消息
        if (unreadcnt > 0) {
    ...
    public Set<String> getUnreadSessionKeyList(){
        return unreadMsgMap.keySet();
    }
    /**
     * 回话是否已经被设定为屏蔽
     * @param unreadEntity
	...
	public void add(MessageEntity msg) {
        ...
        //更新未读消息计数
        if(msg == null){
            logger.i("unread#unreadMgr#add msg is null!");
            return;
        }
        // isFirst场景:出现一条未读消息，出现小红点，需要触发 [免打扰的情况下]
        boolean isFirst = false;
		logger.i("unread#unreadMgr#add unread msg:%s", msg);
    ...
    public void ackReadMsg(MessageEntity entity){
        logger.i("chat#ackReadMsg -> msg:%s", entity);
        String loginId = loginManager.getPub_key();
    ...
    public void ackReadMsg(UnreadEntity unreadEntity){
        logger.i("chat#ackReadMsg -> msg:%s", unreadEntity);
        String loginId = loginManager.getPub_key();
    ...
    public void onNotifyRead(IMMessage.IMMsgDataReadNotify readNotify){
        logger.i("chat#onNotifyRead");
        //发送此信令的用户id
        String trigerId = readNotify.getUserId();
        String loginId = IMLoginManager.instance().getPub_key();
        if(!trigerId.equals(loginId)){
        ...
        String peerId = readNotify.getSessionId();
        ...
        if(unreadSession!=null && unreadSession.getLaststMsgId() <= msgId){
            // 清空会话session
            logger.i("chat#onNotifyRead# unreadSession onLoginOut");
    ...
    public void  readUnreadSession(String sessionKey){
        logger.i("unread#readUnreadSession# sessionKey:%s", sessionKey);
         if(unreadMsgMap.containsKey(sessionKey)){
    ...
    public UnreadEntity findUnread(String sessionKey){
        logger.i("unread#findUnread# buddyId:%s", sessionKey);
