...
public class GroupEntity extends PeerEntity{
   ...
   // private String peerId;
    ...
    public GroupEntity(String id, String peerId, int groupType, String mainName, String avatar, String creatorId, int userCnt, String userList, int version, int status, int created, int updated) {
        this.id = id;
        this.peerId = peerId;
    ...
    public Set<String> getlistGroupMemberIds(){
        ...
        for(int index=0;index < arrayUserIds.length;index++){
            String userId =  arrayUserIds[index];//Integer.parseInt(arrayUserIds[index]);
            result.add(userId);
        }
        return result;
    }
    //todo 入参变为 set【自动去重】
    // 每次都要转换 性能不是太好，todo
    public void setlistGroupMemberIds(List<String> memberList){
        String userList = TextUtils.join(",",memberList);
        setUserCnt(memberList.size());
