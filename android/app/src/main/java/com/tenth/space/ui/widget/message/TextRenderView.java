...
public class TextRenderView extends  BaseMsgRenderView {
    ...
    private TextView quoteMessageContent;//wystan add for quote 210223

    public static TextRenderView inflater(Context context,ViewGroup viewGroup,boolean isMine){
        int resource = isMine?R.layout.tt_mine_text_message_item:R.layout.tt_other_text_message_item;//tt_mine_text_message_item  //wystan test for system tips
    ...
    protected void onFinishInflate() {
        ...
        quoteMessageContent = (TextView) findViewById(R.id.quote_content);
    }
    ...
    public void render(MessageEntity messageEntity, UserEntity userEntity,Context context) {
        ...
        String quoteContent = textMessage.getSimpleShowQuoteContent();
        if(!Utils.isEmpty(quoteContent)) {
            quoteMessageContent.setVisibility(View.VISIBLE);
            quoteMessageContent.setText(Emoparser.getInstance(getContext()).emoCharsequence(quoteContent)); // 所以上层还是处理好之后再给我 Emoparser 处理之后的
        }
        else{
            quoteMessageContent.setVisibility(View.GONE);
        }
    }
    ...
    public TextView getQuoteMessageContent() {
        return quoteMessageContent;
    }
