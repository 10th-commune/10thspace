...
import com.tenth.space.protobuf.IMOther;
...
public class IMHeartBeatManager  extends  IMManager{
    ...
    private final String ACTION_SENDING_HEARTBEAT = "com.tenth.space.imservice.manager.imheartbeatmanager";
    ...
    public void onloginNetSuccess(){
        logger.i("heartbeat#onLocalNetOk");
        IntentFilter intentFilter = new IntentFilter();
        intentFilter.addAction(ACTION_SENDING_HEARTBEAT);
        logger.i("heartbeat#register actions");
    ...
    @Override
    public void reset() {
        logger.i("heartbeat#reset begin");
        try {
            ctx.unregisterReceiver(imReceiver);
            cancelHeartbeatTimer();
            logger.i("heartbeat#reset stop");
        }catch (Exception e){
    ...
    private void scheduleHeartbeat(int seconds){
        logger.i("heartbeat#scheduleHeartbeat every %d seconds", seconds);
        if (pendingIntent == null) {
        ...
        try {
            ...
            IMSocketManager.instance().sendRequest(imHeartBeat,sid,cid,new Packetlistener(timeOut) {
                ...
                public void onSuccess(Object response) {
                    logger.i("heartbeat#心跳成功，链接保活");
                   // Log.i("GTAG","heartbeat#心跳成功，链接保活");
                }

                @Override
                public void onFaild() {
                    Log.i("GTAG","heartbeat#心跳包发送失败");
