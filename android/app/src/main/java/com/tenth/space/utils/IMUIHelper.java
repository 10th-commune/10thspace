...
import com.tenth.space.utils.pinyin.PinYin.PinYinElement;
...
public class IMUIHelper {
    ...
    public static void handleContactItemLongClick(final UserEntity contact, final Context ctx){
        ...
        String[] items = new String[]{ctx.getString(R.string.check_profile),
                ctx.getString(R.string.start_session),"删除好友"};

        final String userId = contact.getPeerId();
...
//                        IMUIHelper.openUserProfileActivity(ctx, userId);
                    ...
                    case 2 :
						//取消关注
                        IMBuddyManager.instance().reqDelFollowUser(contact.getPeerId(),0);
                        break;
                }
            }
        });
    ...
    // 跳转到聊天页面
    public static void openChatActivity(Context ctx, String sessionKey,boolean isNewFriend) {
        Intent intent = new Intent(ctx, MessageActivity.class);
    ...
    //跳转到用户信息页面
    public static void openUserProfileActivity(Context ctx, String contactId,String relation) {
    ...
    public static void  openGroupMemberSelectActivity(Context ctx, String sessionKey,int groupType) {
        Intent intent = new Intent(ctx, GroupMemberSelectActivity.class);
        intent.putExtra(IntentConstant.KEY_SESSION_KEY, sessionKey);
        intent.putExtra(IntentConstant.KEY_GROUP_TYPE,groupType);
        ctx.startActivity(intent);
    }
	...
	public static void displayImage(ImageView imageView,
		...
		logger.i("displayimage#displayImage resourceUri:%s, defeaultResourceId:%d", resourceUri, defaultResId);
...
//			bitmapConfig(Bitmap.Config.RGB_565).// 改善OOM
		...
		ImageLoader.getInstance().displayImage(IMApplication.app.UrlFormat(resourceUri), imageView, options, null);
	}
