...
import com.tenth.space.ui.fragment.UserInfoFragment;
...
import com.tenth.space.ui.activity.PreviewGifActivity;
import com.tenth.space.ui.activity.PreviewMessageImagesActivity;
import com.tenth.space.ui.activity.PreviewTextActivity;
import com.tenth.space.ui.helper.Emoparser;
import com.tenth.space.ui.widget.GifView;
import com.tenth.space.ui.widget.message.GifImageRenderView;
import com.tenth.space.ui.widget.message.SystemMsgRenderView;
import com.tenth.space.utils.CommonUtil;
import com.tenth.space.utils.DateUtil;
import com.tenth.space.utils.FileUtil;
import com.tenth.space.utils.Logger;
import com.tenth.space.ui.widget.SpeekerToast;
import com.tenth.space.ui.widget.message.AudioRenderView;
import com.tenth.space.ui.widget.message.EmojiRenderView;
import com.tenth.space.ui.widget.message.ImageRenderView;
import com.tenth.space.ui.widget.message.MessageOperatePopup;
import com.tenth.space.ui.widget.message.RenderType;
import com.tenth.space.ui.widget.message.TextRenderView;
import com.tenth.space.ui.widget.message.TimeRenderView;
import com.tenth.space.ui.helper.listener.OnDoubleClickListener;
...
public class MessageAdapter extends BaseAdapter {
    ...
    public MessageAdapter(Context ctx, OnQuoteClickListener l) {
        this.ctx = ctx;
        mListener = l;
    }
    ...
    public void addItem(final MessageEntity msg) {
        ...
        logger.i("#messageAdapter#addItem");

        if(!msg.getFromId().equals(loginUser.getPeerId())) {
            CharSequence systemtip = needDisplaySystemTip(msg);
            if (null != systemtip)
                msgObjectList.add(systemtip);
        }
    ...
    private CharSequence needDisplaySystemTip(MessageEntity msg) {
        String content = msg.getContent();
        if (!TextUtils.isEmpty(content)) {
            if(content.contains("钱"))
                return BitherjSettings.ALL_SYSTEM_TIPS[1];
            else if (content.contains("肥")) {
                return BitherjSettings.ALL_SYSTEM_TIPS[4];
            }
        }
    ...
    public void loadHistoryList(final List<MessageEntity> historyList) {
        logger.i("#messageAdapter#loadHistoryList");
    ...
    public int getItemViewType(int position) {
        try {
            ...
            if (obj instanceof Integer) {
                ...
                type = RenderType.MESSAGE_TYPE_SYS_TIP;
    ...
    private View systemTipRender(int position, View convertView, ViewGroup parent) {
        SystemMsgRenderView systemMsgRenderView;
        CharSequence sysTip = (CharSequence) msgObjectList.get(position);
        if (null == convertView) {
            systemMsgRenderView = SystemMsgRenderView.inflater(ctx, parent);
        } else {
            // 不用再使用tag 标签了
            systemMsgRenderView = (SystemMsgRenderView) convertView;
        }
        systemMsgRenderView.setContent(sysTip);
...
//                    imageMessage.setLoadStatus(MessageStatus.IMAGE_UNLOAD);//如果是图片已经上传成功呢？
        ...
        imageRenderView.setImageLoadListener(new ImageRenderView.ImageLoadListener() {
            ...
            public void onLoadComplete(String loaclPath) {
                logger.i("chat#pic#save image ok");
                logger.i("pic#setsavepath:%s", loaclPath);
//                imageMessage.setPath(loaclPath);//下载的本地路径不再存储
            ...
            public void onLoadFailed() {
                logger.i("chat#pic#onBitmapFailed");
    ...
    private View audioMsgRender(final int position, View convertView, final ViewGroup parent, final boolean isMine) {
        ...
        audioRenderView.setBtnImageListener(new AudioRenderView.BtnImageListener() {
            ...
            public void onClickUnread() {
                logger.i("chat#audio#set audio meessage read status");
    ...
    private View textMsgRender(final int position, View convertView, final ViewGroup viewGroup, final boolean isMine) {
        ...
        final TextView quoteTextView = textRenderView.getQuoteMessageContent();
        quoteTextView.setOnTouchListener(new OnDoubleClickListener() {
            @Override
            public void onDoubleClick(View view) {
                mListener.onQuoteDataTouch();
                new quotePopupWindows(ctx, textRenderView, textMessage.getQuoteContent());
            }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        try {
            ...
            switch (renderType) {
                ...
                case MESSAGE_TYPE_SYS_TIP:
                    convertView = systemTipRender(position, convertView, parent);
                    break;
    ...
    public interface OnQuoteClickListener {
        void onQuoteClick(String strContent);
        void onQuoteDataTouch();
    }
    ...
    private class OperateItemClickListener
        ...
        public void onCopyClick() {
            try {
                ...
                logger.i("menu#onCopyClick content:%s", mMsgInfo.getContent());
        ...
        public void onQuoteClick() {
            logger.i("chat#wystan quote click");
            if (mType == DBConstant.SHOW_ORIGIN_TEXT_TYPE) {
                mListener.onQuoteClick(imService.getContactManager().findContact(mMsgInfo.getFromId()).getMainName() + " : " + mMsgInfo.getContent());
            }
        }
    }

    public class quotePopupWindows extends PopupWindow {

        public quotePopupWindows(Context mContext, View parent, final String quoteData) {
            View view = View.inflate(mContext, R.layout.popup_quote, null);
            setContentView(view);

            TextView quoteEdt = (TextView) view.findViewById(R.id.quote_edt);
            quoteEdt.setText(quoteData);
            quoteEdt.setOnClickListener(new View.OnClickListener() {
                @Override
