...
//        oks.setSiteUrl("http://sharesdk.cn");
        ...
        //关闭sso授权
        oks.disableSSOWhenAuthorize();
        String s1 = "http://mobile.10thcommune.com/blog/" + mBlogEntity.getBlogId()+"/tenth";
        // title标题，印象笔记、邮箱、信息、微信、人人网、QQ和QQ空间使用
        oks.setTitle("第十空间");
        // titleUrl是标题的网络链接，仅在Linked-in,QQ和QQ空间使用
        oks.setTitleUrl(s1);
        // text是分享文本，所有平台都需要这个字段
        oks.setText(mBlogEntity.getBlogText());
        //分享网络图片，新浪微博分享网络图片需要通过审核后申请高级写入接口，否则请注释掉测试新浪微博
...
//                isRefresh = true;
    ...
    private void initHeadData() {
       ...
       // ImageLoaderUtil.instance().displayImage(IMApplication.app.UrlFormat(mBlogEntity.getAvatarUrl()), mHeadHolder.mIvHeadIcon, ImageLoaderUtil.getAvatarOptions(10, 0));
        ...
        mHeadHolder.mIvHeadIcon.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
               ...
               // IMUIHelper.openUserProfileActivity(CommentActivity.this, (int) writerUserId1,relation);
            }
        });
        //手机型号
       ...
       // mHeadHolder.mZanCnt.setText(mBlogEntity.getLikeCnt() + "");
        ...
        //点击举报
        mHeadHolder.iv_report.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Utils.showPopupWindow(CommentActivity.this,mBlogEntity.getWriterUserId(),10);
            }
        });
    }
