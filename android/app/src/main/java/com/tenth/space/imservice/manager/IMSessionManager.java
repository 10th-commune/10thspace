...
public class IMSessionManager extends IMManager {
    ...
    public void onNormalLoginOk() {
        logger.i("recent#onLogin Successful");
        onLocalLoginOk();
        onLocalNetOk();
    }
    ...
    public void onLocalLoginOk(){
        ...
        for(SessionEntity sessionInfo:sessionInfoList){
            ...
            if(sessionInfo.getPeerId().equals(publicGroup.getPeerId())) {
                bFirstIn = false;
            }
        }

        if(bFirstIn){
            if(publicGroup.getlistGroupMemberIds().contains(imLoginManager.getPub_key()))
                updatePublicSession(publicGroup);
        }
    ...
    public void onLocalNetOk(){
        ...
        logger.i("session#更新时间:%d",latestUpdateTime);
    ...
    public void onRepRecentContacts(IMBuddy.IMRecentContactSessionRsp recentContactSessionRsp) {
        ...
        String userId = recentContactSessionRsp.getUserId();
        List<IMBaseDefine.ContactSessionInfo> contactSessionInfoList = recentContactSessionRsp.getContactSessionListList();
        logger.i("contact#user:%s  cnt:%d",userId,contactSessionInfoList.size());
        /**更新最近联系人列表*/
        ...
        logger.i("session#onRepRecentContacts is ready,now broadcast");
    ...
    public  void reqRemoveSession(RecentInfo recentInfo) {
        ...
        String loginId = imLoginManager.getLoginId();
    ...
    public void updateSession(GroupEntity entity){
        logger.i("recent#updateSession GroupEntity:%s", entity);
    ...
    /**第一次加入公共群组时候的更新 */
    public void updatePublicSession(GroupEntity entity){
        logger.i("recent#updateSession GroupEntity:%s", entity);
        SessionEntity sessionEntity = new SessionEntity();
        sessionEntity.setLatestMsgType(DBConstant.MSG_TYPE_GROUP_TEXT);
        sessionEntity.setUpdated(entity.getUpdated());
        sessionEntity.setCreated(entity.getCreated());
        sessionEntity.setLatestMsgData("欢迎加入space");
        sessionEntity.setTalkId(entity.getCreatorId());
        sessionEntity.setLatestMsgId(0);
        sessionEntity.setPeerId(entity.getPeerId());
        sessionEntity.setPeerType(DBConstant.SESSION_TYPE_GROUP);
    ...
    public void updateSession(MessageEntity msg) {
        logger.i("recent#updateSession msg:%s", msg);
        if (msg == null) {
            logger.i("recent#updateSession is end,cause by msg is null");
            return;
        }
        ...
        if (sessionEntity == null) {
            logger.i("session#updateSession#not found msgSessionEntity");
            ...
            logger.i("session#updateSession#msgSessionEntity already in Map");
    ...
    public List<RecentInfo> getRecentListInfo(){
        ...
        Map<String,UserEntity> userMap = IMContactManager.instance().getUserMap();
       ...
       //过滤 如果未读消息的会话已经不存在，清除未读 (bug出现过，怎么导致的未知)
        Set<String> unreadSessionKeyList = IMUnreadMsgManager.instance().getUnreadSessionKeyList();
        if(unreadSessionKeyList.size()>0){
            for(String key:unreadSessionKeyList){
                if(!sessionMap.containsKey(key)){
                    UnreadEntity unreadEntity = unreadMsgMap.get(key);
                    IMUnreadMsgManager.instance().ackReadMsg(unreadEntity);
                }
            }
        }
    ...
    public PeerEntity findPeerEntity(String sessionKey){
        ...
        String peerId = sessionInfo[1];//Integer.parseInt(sessionInfo[1]);
        switch (peerType){
