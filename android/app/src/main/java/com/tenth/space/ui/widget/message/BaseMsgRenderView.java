...
public abstract class BaseMsgRenderView extends RelativeLayout{
    ...
    public void render(final MessageEntity entity, UserEntity userEntity,final Context ctx){
       ...
       if(!isMine){
        ...
        final String userId = userEntity.getPub_key();
        portrait.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                UserEntity userEntity = IMContactManager.instance().findContact(userId);
                String relation= "";
                if(userEntity !=null){
                    relation = userEntity.getRelation();
                }
               // IMUIHelper.openUserProfileActivity(getContext(),userId,relation);
            }
        });
        /**头像事件 end*/
