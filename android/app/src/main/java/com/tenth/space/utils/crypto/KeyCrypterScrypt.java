...
public class KeyCrypterScrypt implements KeyCrypter, Serializable {
    private static final Logger log = Logger.getLogger(PrivateKeyUtil.class);//LoggerFactory.getLogger(KeyCrypterScrypt.class);
    ...
    public KeyCrypterScrypt(byte[] salt) {
        ...
        if (salt == null
                || salt.length == 0) {
            log.w("You are using a ScryptParameters with no salt. Your encryption may be vulnerable to a dictionary attack.");
        }
    }
    ...
    public KeyParameter deriveKey(CharSequence password) throws KeyCrypterException {
        ...
        if(null != PrivateKeyUtil.getDerivedKey()) {
            return PrivateKeyUtil.getDerivedKey();
        }
