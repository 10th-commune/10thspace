...
typedef hash_map<string, CUserInfo*> UserInfoMap_t;
static UserInfoMap_t g_user_map;

CUserInfo* GetUserInfo(string user_id)
{
...
void CRouteConn::_HandleUserStatusUpdate(CImPdu* pPdu)
{
	...
	log("HandleUserStatusUpdate, status=%u, uid=%s, client_type=%u ", user_status, user_id.c_str(), client_type);
...
void CRouteConn::_HandleUsersStatusRequest(CImPdu* pPdu)
{
	...
	string request_id = msg.user_id();
	uint32_t query_count = msg.user_id_list_size();
	log("HandleUserStatusReq, req_id=%s, query_count=%u ", request_id.c_str(), query_count);
    ...
    for(uint32_t i = 0; i < query_count; i++)
	{
        ...
        string user_id = msg.user_id_list(i);
        user_stat->set_user_id(user_id);
        CUserInfo* pUser = GetUserInfo(user_id);
        if (pUser) {
 ...
 */
void CRouteConn::_UpdateUserStatus(string user_id, uint32_t status, uint32_t client_type)
{
    CUserInfo* pUser = GetUserInfo(user_id);
    if (pUser) {
...
void CRouteConn::_SendPduToUser(string user_id, CImPdu* pPdu, bool bAll)
{
    CUserInfo* pUser = GetUserInfo(user_id);
    if (pUser)
    {
