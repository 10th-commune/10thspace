...
void CRouteServConn::OnConfirm()
{
    ...
    for (list<user_stat_t>::iterator it = online_user_list.begin(); it != online_user_list.end(); it++) {
        ...
        server_user_stat->set_user_id(user_stat.user_id);//cita modify
...
void CRouteServConn::_HandleKickUser(CImPdu* pPdu)
{
	...
	log("HandleKickUser, user_id=%s, client_type=%s, reason=%u. ", user_id.c_str(), client_type, reason);
...
void CRouteServConn::_HandleStatusNotify(CImPdu* pPdu)
{
	...
	log("HandleFriendStatusNotify, user_id=%s, status=%u ", user_stat.user_id().c_str(), user_stat.status());
...
void CRouteServConn::_HandleMsgData(CImPdu* pPdu)
{
	...
	log("HandleMsgData, %s->%s, msg_id=%u. ", from_user_id.c_str(), to_user_id.c_str(), msg_id);
    
    
    CImUser* pFromImUser = CImUserManager::GetInstance()->GetImUserById(from_user_id);
    if (pFromImUser)
    {
        pFromImUser->BroadcastPdu(pPdu,NULL);
    }
...
void CRouteServConn::_HandleMsgReadNotify(CImPdu *pPdu)
{
    ...
    string req_id = msg.user_id();
    string session_id = msg.session_id();
    uint32_t msg_id = msg.msg_id();
    uint32_t session_type = msg.session_type();
    
    log("HandleMsgReadNotify, user_id=%s, session_id=%s, session_type=%u, msg_id=%u. ", req_id.c_str(), session_id.c_str(), session_type, msg_id);
...
void CRouteServConn::_HandleP2PMsg(CImPdu* pPdu)
{
	...
	log("HandleP2PMsg, %s->%s ", from_user_id.c_str(), to_user_id.c_str());
...
void CRouteServConn::_HandleUsersStatusResponse(CImPdu* pPdu)
{
	...
	log("HandleUsersStatusResp, user_id=%s, query_count=%u ", user_id.c_str(), result_count);
    ...
    else if (attach_data.GetType() == ATTACH_TYPE_PDU_FOR_PUSH)
    {
        ...
        if (user_stat.status() == IM::BaseDefine::USER_STATUS_ONLINE)
        {
            ...
            log("HandleUsersStatusResponse, user id: %s, push type: normal. ", user_stat.user_id().c_str());
        }
        else
        {
...
void CRouteServConn::_HandleRemoveSessionNotify(CImPdu *pPdu)
{
    ...
    string user_id = msg.user_id();
    string session_id = msg.session_id();
    log("HandleRemoveSessionNotify, user_id=%s, session_id=%s ", user_id.c_str(), session_id.c_str());
    CImUser* pUser = CImUserManager::GetInstance()->GetImUserById(user_id);
    if (pUser)
    {
        pUser->BroadcastPdu(pPdu);
...
void CRouteServConn::_HandlePCLoginStatusNotify(CImPdu *pPdu)
{
    ...
    log("HandlePCLoginStatusNotify, user_id=%s, login_status=%u ", user_id.c_str(), login_status);
...
void CRouteServConn::_HandleSignInfoChangedNotify(CImPdu* pPdu) {
        ...
        log("HandleSignInfoChangedNotify, changed_user_id=%s, sign_info=%s ", msg.changed_user_id().c_str(), msg.sign_info().c_str());
