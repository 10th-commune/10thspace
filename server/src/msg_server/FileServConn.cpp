...
void CFileServConn::_HandleFileMsgTransRsp(CImPdu* pPdu)
{
    ...
    log("HandleFileMsgTransRsp, result: %u, from_user_id: %s, to_user_id: %s, file_name: %s, \
        task_id: %s, trans_mode: %u. ", result, from_id.c_str(), to_id.c_str(),
    ...
    if (result == 0)
    {
        ...
        else{//send to route server 
         //wystan modify for invaild send 200612
