...
CLoginServConn* get_login_serv_conn()
{
	CLoginServConn* pConn = NULL;
...
void CLoginServConn::HandlePdu(CImPdu* pPdu)
{
	...
	switch (pPdu->GetCommandId()) {
		case CID_BUDDY_LIST_ALL_ONLINE_USER_CNT_RESPONSE:
			_HandleAllOnlineUserCntResponse(pPdu);
			break;
...
void CLoginServConn::_HandleAllOnlineUserCntResponse(CImPdu* pPdu)
{
	IM::Buddy::IMALLOnlineUserCntRsp msg;
	...
	log("all online user cnt %d", msg.online_user_cnt());
