...
private:
    ...
    void _HandleGetStudyTimeResponse(CImPdu *pPdu);

    void _HandleAddFriendUnreadCntResponse(CImPdu *pPdu);
    void _HandleGetAddFriendDataResponse(CImPdu *pPdu);

    void _HandleSendSysMsgResponse(CImPdu *pPdu);
    void _HandleGetUnreadSysMsgCntResponse(CImPdu *pPdu);
    void _HandleGetSysMsgDataResponse(CImPdu *pPdu);

    //cita add:响应获取在线用户详情
    void _HandleGetAllOnlineUserResponse(CImPdu *pPdu);
private:
	bool 		m_bOpen;
	uint32_t	m_serv_idx;
};
