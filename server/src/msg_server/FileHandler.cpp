...
void CFileHandler::HandleClientFileRequest(CMsgConn* pMsgConn, CImPdu* pPdu)
{
    ...
    string from_id = pMsgConn->GetUserId();
    string to_id = msg.to_user_id();
    string file_name = msg.file_name();
    uint32_t file_size = msg.file_size();
    uint32_t trans_mode = msg.trans_mode();
    log("HandleClientFileRequest, %s->%s, fileName: %s, trans_mode: %u.", from_id.c_str(), to_id.c_str(), file_name.c_str(), trans_mode);
...
{
    string req_user_id = pMsgConn->GetUserId();
    log("HandleClientFileHasOfflineReq, req_id=%s   ", req_user_id.c_str());
...
void CFileHandler::HandleClientFileAddOfflineReq(CMsgConn* pMsgConn, CImPdu* pPdu)
{
    ...
    string file_name = msg.file_name();
    uint32_t file_size = msg.file_size();
    log("HandleClientFileAddOfflineReq, %s->%s, task_id: %s, file_name: %s, size: %u  ",
        from_id.c_str(), to_id.c_str(), task_id.c_str(), file_name.c_str(), file_size);
...
void CFileHandler::HandleClientFileDelOfflineReq(CMsgConn* pMsgConn, CImPdu* pPdu)
{
    ...
    string from_id = msg.from_user_id();
    string to_id = msg.to_user_id();
    string task_id = msg.task_id();
    log("HandleClientFileDelOfflineReq, %s->%s, task_id=%s ", from_id.c_str(), to_id.c_str(), task_id.c_str());
...
void CFileHandler::HandleFileNotify(CImPdu* pPdu)
{
    ...
    log("HandleFileNotify, from_id: %s, to_id: %s, file_name: %s, task_id: %s, trans_mode: %u,\
        offline_ready: %u. ", from_user_id.c_str(), to_user_id.c_str(), file_name.c_str(), task_id.c_str(),trans_mode, offline_ready);
