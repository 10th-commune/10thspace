...
void CGroupChat::HandleClientGroupNormalRequest(CImPdu* pPdu, CMsgConn* pFromConn)
{
    ...
    string user_id = pFromConn->GetUserId();
    log("HandleClientGroupNormalRequest, user_id=%s. ", user_id.c_str());
...
void CGroupChat::HandleGroupNormalResponse(CImPdu* pPdu)
{
    ...
    log("HandleGroupNormalResponse, user_id=%s, group_cnt=%u. ", user_id.c_str(), group_cnt);
...
void CGroupChat::HandleClientGroupInfoRequest(CImPdu *pPdu, CMsgConn* pFromConn)
{
    ...
    string user_id = pFromConn->GetUserId();
    uint32_t group_cnt = msg.group_version_list_size();
    log("HandleClientGroupInfoRequest, user_id=%s, group_cnt=%u. ", user_id.c_str(), group_cnt);
...
void CGroupChat::HandleGroupInfoResponse(CImPdu* pPdu)
{
    ...
    log("HandleGroupInfoResponse, user_id=%s, group_cnt=%u. ", user_id.c_str(), group_cnt);
    ...
    if (pduAttachData.GetPduLength() > 0 && group_cnt > 0)
    {
        ...
        string group_id = group_info.group_id();
        log("GroupInfoRequest is send by server, group_id=%s ", group_id.c_str());
        
        std::set<string> group_member_set;
        for (uint32_t i = 0; i < group_info.group_member_list_size(); i++)
        {
            string member_user_id = group_info.group_member_list(i);
            group_member_set.insert(member_user_id);
        }
        if (group_member_set.find(user_id) == group_member_set.end())
        {
            log("user_id=%s is not in group, group_id=%s. ", user_id.c_str(), group_id.c_str());
            return;
        }
...
void CGroupChat::HandleGroupMessage(CImPdu* pPdu)
{
	...
	string to_group_id = msg.to_session_id();
	string msg_data = msg.msg_data();
    uint32_t msg_id = msg.msg_id();
    if (msg_id == 0) {
        log("HandleGroupMsg, write db failed, %s->%s. ", from_user_id.c_str(), to_group_id.c_str());
        return;
    }
    uint8_t msg_type = msg.msg_type();
    CDbAttachData attach_data((uchar_t*)msg.attach_data().c_str(), msg.attach_data().length());

    log("HandleGroupMsg, %s->%s, msg id=%u. ", from_user_id.c_str(), to_group_id.c_str(), msg_id);
...
void CGroupChat::HandleGroupMessageBroadcast(CImPdu *pPdu)
{
    ...
    string msg_data = msg.msg_data();
    uint32_t msg_id = msg.msg_id();
    log("HandleGroupMessageBroadcast, %s->%s, msg id=%u. ", from_user_id.c_str(), to_group_id.c_str(), msg_id);
...
void CGroupChat::HandleClientGroupCreateRequest(CImPdu* pPdu, CMsgConn* pFromConn)
{
	...
	string req_user_id = pFromConn->GetUserId();
    string group_name = msg.group_name();
    uint32_t group_type = msg.group_type();
	
    if (group_type == IM::BaseDefine::GROUP_TYPE_NORMAL) {
        log("HandleClientGroupCreateRequest, create normal group failed, req_id=%s, group_name=%s. ", req_user_id.c_str(), group_name.c_str());
        return;
    }
    
	string group_avatar = msg.group_avatar();
	uint32_t user_cnt = msg.member_id_list_size();
	log("HandleClientGroupCreateRequest, req_id=%s, group_name=%s, avatar_url=%s, user_cnt=%u ",
			req_user_id.c_str(), group_name.c_str(), group_avatar.c_str(), user_cnt);
...
void CGroupChat::HandleGroupCreateResponse(CImPdu* pPdu)
{
	...
	string group_id = msg.group_id();
	string group_name = msg.group_name();
	uint32_t user_cnt = msg.user_id_list_size();
	log("HandleGroupCreateResponse, req_id=%s, result=%u, group_id=%s, group_name=%s, member_cnt=%u. ", user_id.c_str(), result, group_id.c_str(), group_name.c_str(), user_cnt);
...
void CGroupChat::HandleClientGroupChangeMemberRequest(CImPdu* pPdu, CMsgConn* pFromConn)
{
	...
	string group_id = msg.group_id();
	uint32_t user_cnt = msg.member_id_list_size();
	log("HandleClientChangeMemberReq, change_type=%u, req_id=%s, group_id=%s, user_cnt=%u ",
			change_type, req_user_id.c_str(), group_id.c_str(), user_cnt);
...
void CGroupChat::HandleGroupChangeMemberResponse(CImPdu* pPdu)
{
	...
	log("HandleChangeMemberResp, change_type=%u, req_id=%s, group_id=%s, result=%u, chg_usr_cnt=%u, cur_user_cnt=%u. ",
			change_type, user_id.c_str(), group_id.c_str(), result, chg_user_cnt, cur_user_cnt);
	...
	if (!result) {
        ...
        for (uint32_t i = 0; i < chg_user_cnt; i++)
        {
            string to_user_id = msg.chg_user_id_list(i);
            _SendPduToUser(&pdu, to_user_id, pFromConn);
        }
        for (uint32_t i = 0; i < cur_user_cnt; i++) {
            string to_user_id = msg.cur_user_id_list(i);
            _SendPduToUser(&pdu, to_user_id, pFromConn);
        }
    }
}
...
void CGroupChat::HandleGroupChangeMemberBroadcast(CImPdu* pPdu)
{
	...
	uint32_t chg_user_cnt = msg.chg_user_id_list_size();
    uint32_t cur_user_cnt = msg.cur_user_id_list_size();
	log("HandleChangeMemberBroadcast, change_type=%u, group_id=%s, chg_user_cnt=%u, cur_user_cnt=%u. ", change_type, group_id.c_str(), chg_user_cnt, cur_user_cnt);
...
void CGroupChat::HandleClientGroupShieldGroupRequest(CImPdu *pPdu, CMsgConn *pFromConn)
{
    ...
    string group_id = msg.group_id();
    string user_id = pFromConn->GetUserId();
    log("HandleClientGroupShieldGroupRequest, user_id: %s, group_id: %s, shield_status: %u. ",
        user_id.c_str(), group_id.c_str(), shield_status);
...
void CGroupChat::HandleGroupShieldGroupResponse(CImPdu *pPdu)
{
    ...
    string user_id = msg.user_id();
    string group_id = msg.group_id();
    log("HandleGroupShieldGroupResponse, result: %u, user_id: %s, group_id: %s. ", result,
...
void CGroupChat::HandleGroupGetShieldByGroupResponse(CImPdu *pPdu)
{
    ...
    for (uint32_t i = 0; i < shield_status_list_cnt; i++)
    {
        ...
        {
            log("user_id: %s shield group, group id: %s. ", shield_status.user_id().c_str(), shield_status.group_id().c_str());
        }
    }
    CImPdu pdu;
    pdu.SetPBMsg(&msg2);
...
void CGroupChat::_SendPduToUser(CImPdu* pPdu, string user_id, CMsgConn* pReqConn)
{
    if(!pPdu) {
        return;
    }
...
//cita add: 转发到dbserver获取群信息
void CGroupChat::HandleGetRecommandGroudListRequest(CImPdu* pPdu,CMsgConn* pFromConn)
{
    IM::Group::IMRecommandGroupListReq msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    log("user id = %s",msg.user_id().c_str());
    CDBServConn* pDbConn = get_db_serv_conn();
	if (pDbConn) {
        CDbAttachData attach_data(ATTACH_TYPE_HANDLE, pFromConn->GetHandle(), 0);
        msg.set_attach_data(attach_data.GetBuffer(), attach_data.GetLength());
...
void CGroupChat::HandleGetRecommandGroudListResponse(CImPdu * pPdu)
{
    IM::Group::IMRecommandGroupListRsp msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(),pPdu->GetBodyLength()));
    string user_id = msg.user_id();
    CDbAttachData attach_data((uchar_t*)msg.attach_data().c_str(), msg.attach_data().length());
    ...
    if(pMsgConn)
    {
        log("recommand group size = %d",msg.group_info_list_size());
        pMsgConn->SendPdu(pPdu);
    }
