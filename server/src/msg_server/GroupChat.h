...
public:
	...
	//cita add:获取推荐群
	void HandleGetRecommandGroudListRequest(CImPdu* pPdu,CMsgConn* pFromConn);
	void HandleGetRecommandGroudListResponse(CImPdu* pPdu);

private:
	CGroupChat() {}	// for singleton;

	void _SendPduToUser(CImPdu* pPdu, string user_id, CMsgConn* pReqConn = NULL);
private:
