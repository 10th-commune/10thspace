...
public:
    ...
    uint32_t getUser(string nUserId, uint32_t nTime =0);
    uint32_t getUserInfo(string nUserId, list<string>& lsUserId);
    uint32_t sendMessage(string nFromId, string nToId, IM::BaseDefine::MsgType nType, const string& strMsgData);
    uint32_t getUnreadMsgCnt(string nUserId);
    uint32_t getRecentSession(string nUserId, uint32_t nLastTime);
    uint32_t getMsgList(string nUserId, IM::BaseDefine::SessionType nType, string nPeerId, uint32_t nMsgId, uint32_t nMsgCnt);
    uint32_t sendMsgAck(string nUserId, string nPeerId, IM::BaseDefine::SessionType nType, uint32_t nMsgId);
public:
