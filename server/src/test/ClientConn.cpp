...
uint32_t ClientConn::getUser(string nUserId, uint32_t nTime)
{
    CImPdu cPdu;
...
uint32_t ClientConn::getUserInfo(string nUserId, list<string>& lsUserId)
{
    CImPdu cPdu;
...
uint32_t ClientConn::sendMessage(string nFromId, string nToId, IM::BaseDefine::MsgType nType, const string& strMsgData)
{
    CImPdu cPdu;
...
uint32_t ClientConn::getUnreadMsgCnt(string nUserId)
{
    CImPdu cPdu;
...
uint32_t ClientConn::getRecentSession(string nUserId, uint32_t nLastTime)
{
    CImPdu cPdu;
...
uint32_t ClientConn::getMsgList(string nUserId, IM::BaseDefine::SessionType nType, string nPeerId, uint32_t nMsgId, uint32_t nMsgCnt)
{
    CImPdu cPdu;
...
uint32_t ClientConn::sendMsgAck(string nUserId, string nPeerId, IM::BaseDefine::SessionType nType, uint32_t nMsgId)
{
    CImPdu cPdu;
...
void ClientConn::_HandleSendMsg(CImPdu* pPdu)
{
    ...
    if(msgResp.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
    {
        string nSendId = msgResp.user_id();
        string nRecvId = msgResp.session_id();
...
void ClientConn::_HandleUnreadCnt(CImPdu* pPdu)
{
    ...
    if(msgResp.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
    {
        ...
        string nUserId = msgResp.user_id();
...
void ClientConn::_HandleMsgList(CImPdu *pPdu)
{
    ...
    if(msgResp.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
    {
        ...
        string nPeerId = msgResp.session_id();
