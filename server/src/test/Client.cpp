...
void CClient::connect()
{
    ...
    string strUrl = m_strLoginDomain + "/login_server";
    string strResp;
    ...
    try {
        ...
        nPort = std::stoul(value["port"].asString());
        
    } catch (std::runtime_error msg) {
        printf("login falied. get json error:%s %s\n", strResp.c_str(),msg.what());
        PROMPTION;
        return;
    }
...
{
    string nUserId = m_cSelfInfo.user_id();
...
void CClient::onGetChangedUser(uint32_t nSeqNo, const list<IM::BaseDefine::UserInfo> &lsUser)
{
    for(auto it=lsUser.begin(); it!=lsUser.end(); ++it)
    {
        ...
        *pUserInfo = *it;
        string nUserId = pUserInfo->user_id();
        ...
        if(it->status() != 3)
        {
            ...
            {
                m_mapId2UserInfo.insert(pair<string, IM::BaseDefine::UserInfo*>(nUserId, pUserInfo));
            }
            else
            {
                delete it1->second;
...
uint32_t CClient::getUserInfo(list<string>& lsUserId)
{
    string nUserId = m_cSelfInfo.user_id();
...
uint32_t CClient::sendMsg(string nToId, IM::BaseDefine::MsgType nType, const string &strMsg)
{
    string nFromId = m_cSelfInfo.user_id();
    return g_pConn->sendMessage(nFromId, nToId, nType, strMsg);
}

void CClient::onSendMsg(uint32_t nSeqNo, string nSendId, string nRecvId, IM::BaseDefine::SessionType nType, uint32_t nMsgId)
{
...
void CClient::onGetUnreadMsgCnt(uint32_t nSeqNo, string nUserId, uint32_t nTotalCnt, const list<IM::BaseDefine::UnreadInfo>& lsUnreadCnt)
{
...
void CClient::onGetRecentSession(uint32_t nSeqNo, string nUserId, const list<IM::BaseDefine::ContactSessionInfo> &lsSession)
{
    
}

uint32_t CClient::getMsgList(IM::BaseDefine::SessionType nType, string nPeerId, uint32_t nMsgId, uint32_t nMsgCnt)
{
    string nUserId = m_cSelfInfo.user_id();
    return g_pConn->getMsgList(nUserId, nType, nPeerId, nMsgId, nMsgCnt);
}

void CClient::onGetMsgList(uint32_t nSeqNo, string nUserId, string nPeerId, IM::BaseDefine::SessionType nType, uint32_t nMsgId, uint32_t nMsgCnt, const list<IM::BaseDefine::MsgInfo> &lsMsg)
{
    
}

void CClient::onRecvMsg(uint32_t nSeqNo, string nFromId, string nToId, uint32_t nMsgId, uint32_t nCreateTime, IM::BaseDefine::MsgType nMsgType, const string &strMsgData)
{
