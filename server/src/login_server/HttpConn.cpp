...
extern string strFastDfs;
extern string strApkDownloadUrl;
extern string strApkVersion;
...
void CHttpConn::OnRead()
{
	...
	if (m_cHttpParser.IsReadAll()) {
		...
		if (strncmp(url.c_str(), "/login_server", 11) == 0) {
...
void CHttpConn::_HandleMsgServRequest(string& url, string& post_data)
{
    ...
    if (it_min_conn == g_msg_serv_info.end()) {
        ...
        value["fastDfs"] = strFastDfs;
        Json::Value updateValue;
        updateValue["version"] = strApkVersion;
        updateValue["apkDownloadUrl"] = strApkDownloadUrl;
