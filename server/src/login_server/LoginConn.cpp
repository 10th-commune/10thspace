...
map<uint32_t, user_friend_map_t*> g_user_online_map;
...
void CLoginConn::_HandleAllOnlineUserCntRequest(CImPdu* pPdu)
{
	 IM::Buddy::IMALLOnlineUserCntReq msg;
	 IM::Buddy::IMALLOnlineUserCntRsp msgResp;
	 msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength());

	 log("total online user cnt %d",g_total_online_user_cnt);

	 msgResp.set_user_id(msg.user_id());
	 msgResp.set_online_user_cnt(g_total_online_user_cnt);
	 msgResp.set_attach_data(msg.attach_data());
	 CImPdu pdu;
	 pdu.SetPBMsg(&msgResp);
	 pdu.SetServiceId(SID_BUDDY_LIST);
	 pdu.SetCommandId(CID_BUDDY_LIST_ALL_ONLINE_USER_CNT_RESPONSE);
