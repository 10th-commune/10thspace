...
{
	//if(8600 == m_local_port || 8601 == m_local_port)//wystan add for debug 200525
	//	log("OnRead myport=%d, m_state=%d\n",m_local_port,m_state);
	...
	else
	{
		...
		{
			//log("Close m_socket=%d,myport=%d, m_state=%d\n",m_socket,m_local_port,m_state);
			m_state = SOCKET_STATE_CLOSING;//wystan add for client ctrl+C quit 
			m_callback(m_callback_data, NETLIB_MSG_CLOSE, (net_handle_t)m_socket, NULL);
		}
		else
		{
			if(m_state!=SOCKET_STATE_CLOSING)//wystan add for client ctrl+C quit 
...
void CBaseSocket::OnWrite()
{
	...
	{
		//log("write m_socket=%d myport=%d, m_state=%d\n",m_socket,m_local_port,m_state);
		if(m_state!=SOCKET_STATE_CLOSING)//wystan add for client ctrl+C quit 
...
void CBaseSocket::_AcceptNewSocket()
{
	...
	while ( (fd = accept(m_socket, (sockaddr*)&peer_addr, &addr_len)) != INVALID_SOCKET )
	{
		...
		log("AcceptNewSocket myport=%d, socket=%d from %s:%d\n",m_local_port, fd, ip_str, port);
