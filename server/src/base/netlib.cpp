...
static void mySigSegv(int signum, siginfo_t *info, void *ptr)
{
	log("Server Catch Segmentation fault Error.");
	void *array[25];
	int nSize = backtrace(array, sizeof(array)/sizeof(array[0]));
	for (int i=nSize-3; i>=2; i--){
		if (NULL != ptr){
			ucontext_t* ptrUC = (ucontext_t*)ptr;
			int *pgregs = (int*)(&(ptrUC->uc_mcontext.gregs));
			int eip = pgregs[REG_EIP];
			if (eip != (long)(array[i])){
				log("signal[%d] catched when running code at %x", signum, (char*)array[i] - 1);
			}
		}
		else{
			log("signal[%d] catched when running code at unknown address", signum);
		} //addr2line 8055574  -sCf -e XXXXX
	}
	log("process abort now", signum);
	abort();
}

void setupSigSegvHandler()
{
	struct sigaction act;
	int sig = SIGSEGV;
	sigemptyset(&act.sa_mask);
	act.sa_sigaction = mySigSegv;
	act.sa_flags = SA_SIGINFO;
	if(sigaction(sig, &act, NULL)<0){
		log("setupSigSegvHandler Failed.");	
	}
	else{
		log("setupSigSegvHandler Successed.");
	}
}
