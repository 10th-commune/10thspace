...
namespace DB_PROXY {
    ...
    //            *pInfo = *it;
            ...
            log("userId=%s, count=%u", nUserId.c_str(), msgResp.offline_file_list_size());
    ...
    void addOfflineFile(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            string nUserId = msg.from_user_id();
            string nToId = msg.to_user_id();
            ...
            log("fromId=%s, toId=%s, taskId=%s, fileName=%s, fileSize=%u", nUserId.c_str(), nToId.c_str(), strTaskId.c_str(), strFileName.c_str(), nFileSize);
        }
    }
    ...
    void delOfflineFile(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            log("fromId=%s, toId=%s, taskId=%s", nUserId.c_str(), nToId.c_str(), strTaskId.c_str());
        }
    }
};