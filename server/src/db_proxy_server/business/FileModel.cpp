...
void CFileModel::getOfflineFile(string userId, list<IM::BaseDefine::OfflineFileInfo>& lsOffline)
{
    CDBManager* pDBManager = CDBManager::getInstance();
    CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_slave");
    if (pDBConn)
    {
        string strSql = "select * from IMTransmitFile where toId='"+(userId) + "' and status=0 order by created";
        ...
        if(pResultSet)
        {
            while (pResultSet->Next())
            {
                ...
                offlineFile.set_from_user_id(pResultSet->GetString("fromId"));
...
void CFileModel::addOfflineFile(string fromId, string toId, string& taskId, string& fileName, uint32_t fileSize)
{
    CDBManager* pDBManager = CDBManager::getInstance();
...
void CFileModel::delOfflineFile(string fromId, string toId, string& taskId)
{
    CDBManager* pDBManager = CDBManager::getInstance();
    CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_master");
    if (pDBConn)
    {
        string strSql = "delete from IMTransmitFile where  fromId='" + (fromId) + "' and toId='"+(toId) + "' and taskId='" + taskId + "'";
        if(pDBConn->ExecuteUpdate(strSql.c_str()))
        {
            log("delete offline file success.%s->%s:%s", fromId.c_str(), toId.c_str(), taskId.c_str());
        }
        else
        {
            log("delete offline file failed.%s->%s:%s", fromId.c_str(), toId.c_str(), taskId.c_str());
        }
        pDBManager->RelDBConn(pDBConn);
    }
    else
