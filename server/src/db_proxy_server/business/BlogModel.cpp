...
uint32_t CBlogModel::sendBlog(string nFromId, uint32_t nCreateTime, uint32_t nMsgId, string& strMsgContent)
{
    bool bRet = false;
    uint32_t blog_id = 0;
...
//page从0开始
void CBlogModel::getBlog(string nUserId, uint32_t update_time, IM::BaseDefine::BlogType type,
	...
	if (pDBConn)
	{
		...
		if(type == IM::BaseDefine::BLOG_TYPE_RCOMMEND){
			...
			strSql = "select IMBlog_0.*,nick,avatar from IMBlog_0,IMUser where fromId=IMUser.id and type=" +
				...
				(nUserId) + " or fromId in (select smallId+bigId-" + (nUserId) +
				" from IMRelationShip where status=" + int2string(RELATION_TYPE_FRIEND) +
				" and (smallId=" + (nUserId) + " or bigId=" + (nUserId) +
			...
			strSql = "select IMBlog_0.*,nick,avatar from IMBlog_0,IMUser where fromId=IMUser.id and type=" +
				int2string(IM::BaseDefine::BLOG_TYPE2_BLOG) + " and fromId in (select smallId+bigId-" + (nUserId) +
				" from IMRelationShip where (bigId="+(nUserId)+" and (status=" +
				int2string(RELATION_TYPE_FOLLOW_SMALL) + " or status=" +
				int2string(RELATION_TYPE_FOLLOW_EACH_OTHER) + ")) or (smallId="+(nUserId)+
		...
		if (pResultSet)
		{
			while (pResultSet->Next())
			{
				...
				pBlog->set_writer_user_id(pResultSet->GetString("fromId"));
	...
	if (pDBConn)
	{
		...
		if (pResultSet)
		{
			while (pResultSet->Next())
			{
				...
				pComment->set_writer_user_id(pResultSet->GetString("fromId"));
...
uint32_t CBlogModel::addBlogComment(string nUserId, uint32_t nBlogId, uint32_t nCreateTime, string& strMsgContent)
{
    bool bRet = false;
