...
namespace DB_PROXY {
    ...
    //            *pContact = *it;
            ...
            log("userId=%s, last_time=%u, count=%u", nUserId.c_str(), nLastTime, msgResp.contact_session_list_size());
    ...
    void deleteRecentSession(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            {
                bool bRet = false;
                string nSessionId = CSessionModel::getInstance()->getSessionId(nUserId, nPeerId, nType, false);
                if (nSessionId != "") {
                ...
                log("userId=%s, peerId=%s, result=%s", nUserId.c_str(), nPeerId.c_str(), bRet?"success":"failed");
            ...
            {
                log("invalied session_type. userId=%s, peerId=%s, seseionType=%u", nUserId.c_str(), nPeerId.c_str(), nType);
            }
        }
        else{
            log("parse pb failed");
