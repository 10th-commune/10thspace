...
void CSessionModel::getRecentSession(string nUserId, uint32_t lastTime, list<IM::BaseDefine::ContactSessionInfo>& lsContact)
{
    CDBManager* pDBManager = CDBManager::getInstance();
    CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_slave");
    if (pDBConn)
    {
        string strSql = "select * from IMRecentSession where userId ='" + (nUserId) + "' and status = 0 and updated >" + int2string(lastTime) + " order by updated desc limit 100";
        ...
        if (pResultSet)
        {
            while (pResultSet->Next())
            {
                ...
                string nPeerId = pResultSet->GetString("peerId");
                ...
                {
                    log("invalid sessionType. userId=%s, peerId=%s, sessionType=%u", nUserId.c_str(), nPeerId.c_str(), nSessionType);
                }
            }
            delete pResultSet;
        }
...
string CSessionModel::getSessionId(string nUserId, string nPeerId, uint32_t nType, bool isAll)
{
    CDBManager* pDBManager = CDBManager::getInstance();
    CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_slave");
    string nSessionId;;
    if(pDBConn)
    {
        string strSql;
        if (isAll) {
            strSql= "select id from IMRecentSession where userId='" + (nUserId) + "' and peerId='" + (nPeerId) + "' and type=" + int2string(nType);
        }
        else
        {
            strSql= "select id from IMRecentSession where userId='" + (nUserId) + "' and peerId='" + (nPeerId) + "' and type=" + int2string(nType) + " and status=0";
        }
        ...
        {
            while (pResultSet->Next()) {
                nSessionId = pResultSet->GetString("id");
            }
            delete pResultSet;
        }
        pDBManager->RelDBConn(pDBConn);
...
bool CSessionModel::updateSession(string nSessionId, uint32_t nUpdateTime)
{
    bool bRet = false;
    CDBManager* pDBManager = CDBManager::getInstance();
    ...
    if (pDBConn)
    {
        string strSql = "update IMRecentSession set `updated`="+int2string(nUpdateTime) + " where id='"+(nSessionId)+"'";
...
bool CSessionModel::removeSession(string nSessionId)
{
    bool bRet = false;
    CDBManager* pDBManager = CDBManager::getInstance();
    CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_master");
    if (pDBConn)
    {
        ...
        string strSql = "update IMRecentSession set status = 1, updated="+int2string(nNow)+" where id='" + (nSessionId)+"'";
        bRet = pDBConn->ExecuteUpdate(strSql.c_str());
        pDBManager->RelDBConn(pDBConn);
    }
    else
...
string CSessionModel::addSession(string nUserId, string nPeerId, uint32_t nType)
{
    string nSessionId;;
    ...
    if (pDBConn)
    {
        if("" != nSessionId)
        {
            string strSql = "update IMRecentSession set status=0, updated=" + int2string(nTimeNow) + " where id='" + (nSessionId)+"'";
        ...
        else
        {
            ...
            if (stmt->Init(pDBConn->GetMysql(), strSql))
            {
                ...
                if (bRet)
                {
                    strSql = "select id from IMRecentSession order by created desc limit 1";
                    CResultSet *pSet = pDBConn->ExecuteQuery(strSql.c_str());
                    if(pSet)
                    {
                        while(pSet->Next())
                         nSessionId = pSet->GetString("id");
                    }
                }
                else
                {
...
void CSessionModel::fillSessionMsg(string nUserId, list<IM::BaseDefine::ContactSessionInfo>& lsContact)
{
    for (auto it=lsContact.begin(); it!=lsContact.end();)
    {
        uint32_t nMsgId = 0;
        string strMsgData;
        IM::BaseDefine::MsgType nMsgType = IM::BaseDefine::MsgType::MSG_TYPE_SINGLE_TEXT;
        string nFromId;
