...
const int g_GroupMessageFlag = 10;
 ...
 */
bool CGroupMessageModel::sendMessage(string nFromId, string nGroupId, IM::BaseDefine::MsgType nMsgType, uint32_t nCreateTime,uint32_t nMsgId, const string& strMsgContent, const string & strSigData)
{
    bool bRet = false;
    if(CGroupModel::getInstance()->isInGroup(nFromId, nGroupId))
    {
        ...
        if (pDBConn)
        {
            string strTableName = "IMGroupMessage_" + int2string(g_GroupMessageFlag % 8);
            string strSql = "insert into " + strTableName + " (`groupId`, `userId`, `msgId`, `content`, `type`, `status`, `updated`, `created`, `contentSig`) "\
            "values(?, ?, ?, ?, ?, ?, ?, ?,?)";
    ...
    {
        log("not in the group.fromId=%s, groupId=%s", nFromId.c_str(), nGroupId.c_str());
    }
    return bRet;
}
 ...
 *  @return 成功返回true，失败返回false
 */
bool CGroupMessageModel::sendAudioMessage(string nFromId, string nGroupId, IM::BaseDefine::MsgType nMsgType, uint32_t nCreateTime, uint32_t nMsgId, const char* pMsgContent, uint32_t nMsgLen)
{
	if (nMsgLen <= 4) {
		return false;
	}
 ...
 *  @return 成功返回true，失败返回false
 */
bool CGroupMessageModel::clearMessageCount(string nUserId, string nGroupId)
{
    bool bRet = false;
    ...
    if (pCacheConn)
    {
        string strGroupKey = (nGroupId) + GROUP_TOTAL_MSG_COUNTER_REDIS_KEY_SUFFIX;
        map<string, string> mapGroupCount;
        bool bRet = pCacheConn->hgetAll(strGroupKey, mapGroupCount);
        pCacheManager->RelCacheConn(pCacheConn);
        if(bRet)
        {
            string strUserKey = (nUserId) + "_" + (nGroupId) + GROUP_USER_MSG_COUNTER_REDIS_KEY_SUFFIX;
 ...
 *  @return 成功返回true，失败返回false
 */
bool CGroupMessageModel::incMessageCount(string nUserId, string nGroupId)
{
    bool bRet = false;
    CacheManager* pCacheManager = CacheManager::getInstance();
    CacheConn* pCacheConn = pCacheManager->GetCacheConn("unread");
 ...
 *  @param lsMsg    消息列表
 */
void CGroupMessageModel::getMessage(string nUserId, string nGroupId, uint32_t nMsgId, uint32_t nMsgCnt, list<IM::BaseDefine::MsgInfo>& lsMsg)
{
    ...
    if (pDBConn)
    {
        ...
        if(nMsgId == 0)
        {
            strSql = "select * from " + strTableName + " where groupId = '" + (nGroupId) + "' and status = 0 and created>="+ int2string(nUpdated) + " order by created desc, id desc limit " + int2string(nMsgCnt);
        }else {
            strSql = "select * from " + strTableName + " where groupId = '" + (nGroupId) + "' and msgId<=" + int2string(nMsgId) + " and status = 0 and created>="+ int2string(nUpdated) + " order by created desc, id desc limit " + int2string(nMsgCnt);
        }
        ...
        if (pResultSet)
        {
            ...
            while(pResultSet->Next())
            {
                ...
                msg.set_from_session_id(pResultSet->GetString("userId"));
                ...
                {
                    log("invalid msgType. userId=%s, groupId=%s, msgType=%u", nUserId.c_str(), nGroupId.c_str(), nMsgType);
                }
            }
            delete pResultSet;
        }
 ...
 */
void CGroupMessageModel::getUnreadMsgCount(string nUserId, uint32_t &nTotalCnt, list<IM::BaseDefine::UnreadInfo>& lsUnreadCount)
{
...
//                log("hget %s : count failed !", strGroupKey.c_str());
            ...
            if(nCount > 0)
            {
                ...
                {
                    log("invalid msgType. userId=%s, groupId=%s, msgType=%u, msgId=%u", nUserId.c_str(), nGroupId.c_str(), nType, nMsgId);
                }
            }
        }
        pCacheManager->RelCacheConn(pCacheConn);
 ...
 *  @return 返回msgId
 */
uint32_t CGroupMessageModel::getMsgId(string nGroupId)
{
    uint32_t nMsgId = 0;
    CacheManager* pCacheManager = CacheManager::getInstance();
    CacheConn* pCacheConn = pCacheManager->GetCacheConn("unread");
    ...
    {
        // TODO
        string strKey = "group_msg_id_" + (nGroupId);
 ...
 */
void CGroupMessageModel::getLastMsg(string nGroupId, uint32_t &nMsgId, string &strMsgData, IM::BaseDefine::MsgType &nMsgType, string& nFromId)
{
    string strTableName = "IMGroupMessage_" + int2string(g_GroupMessageFlag % 8);
    ...
    if (pDBConn)
    {
        string strSql = "select msgId, type,userId, content from " + strTableName + " where groupId = '" + (nGroupId) + "' and status = 0 order by created desc, id desc limit 1";
        ...
        if (pResultSet)
        {
            while(pResultSet->Next()) {
                ...
                nFromId = pResultSet->GetString("userId");
 ...
 */
void CGroupMessageModel::getUnReadCntAll(string nUserId, uint32_t &nTotalCnt)
{
    list<string> lsGroupId;
    CGroupModel::getInstance()->getUserGroupIds(nUserId, lsGroupId, 0);
    uint32_t nCount = 0;
...
void CGroupMessageModel::getMsgByMsgId(string nUserId, string nGroupId, const list<uint32_t> &lsMsgId, list<IM::BaseDefine::MsgInfo> &lsMsg)
{
    if(!lsMsgId.empty())
    {
        if (CGroupModel::getInstance()->isInGroup(nUserId, nGroupId))
        {
            ...
            if (pDBConn)
            {
                ...
                string strSql = "select * from " + strTableName + " where groupId='" + (nGroupId) + "' and msgId in (" + strClause + ") and status=0 and created >= " + int2string(nUpdated) + " order by created desc, id desc limit 100";
                CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
                if (pResultSet)
                {
                    while (pResultSet->Next())
        ...
        {
            log("%s is not in group:%s", nUserId.c_str(), nGroupId.c_str());
        }
    }
    else
    {
...
bool CGroupMessageModel::resetMsgId(string nGroupId)
{
    bool bRet = false;
    CacheManager* pCacheManager = CacheManager::getInstance();
    CacheConn* pCacheConn = pCacheManager->GetCacheConn("unread");
