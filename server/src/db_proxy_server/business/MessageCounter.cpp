...
namespace DB_PROXY {
    ...
    //            *pInfo = *it;
            ...
            log("userId=%s, unreadCnt=%u, totalCount=%u", nUserId.c_str(), msgResp.unreadinfo_list_size(), nTotalCnt);
    ...
    void clearUnreadMsgCounter(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            log("userId=%s, peerId=%s, type=%u", nFromId.c_str(), nUserId.c_str(), nSessionType);
        }
        else
        {
            log("parse pb failed");
    ...
    void setDevicesToken(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if (pCacheConn)
            {
                ...
                string strOldValue = pCacheConn->get("device_"+(nUserId));
                ...
                pCacheConn->set("device_"+(nUserId), strValue);
                string strNewValue = (nUserId);
                pCacheConn->set("device_"+strToken, strNewValue);
            
                log("setDeviceToken. userId=%s, deviceToken=%s", nUserId.c_str(), strToken.c_str());
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if (pCacheConn)
            {
                ...
                for (uint32_t i=0; i<nCnt; ++i) {
                    string strKey = "device_"+(msg.user_id(i));
