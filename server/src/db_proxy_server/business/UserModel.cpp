...
void CUserModel::getChangedId(uint32_t& nLastTime, list<string> &lsIds)
{
    ...
    if (pDBConn)
    {
        ...
        if(pResultSet)
        {
            while (pResultSet->Next()) {
                string nId = pResultSet->GetString("id");
...
void CUserModel::getUsers(list<string> lsIds, list<IM::BaseDefine::UserInfo> &lsUsers)
{
    if (lsIds.empty()) {
        log("list is empty");
        return;
    ...
    if (pDBConn)
    {
        ...
        {
        	if(nLimit > 500)//wystan add for limit select nums 200604
				break;
...
void CUserModel::getUsers2(string nUserId,uint32_t nLastTime,IM::Buddy::IMAllUserRsp& msgResp)
{
    CDBManager* pDBManager = CDBManager::getInstance();
    CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_slave");
    if (pDBConn)
    {
        string  strSql = "select IMUser.* from IMUser,IMRelationShip where ((IMUser.id=IMRelationShip.smallId "
        		"and IMRelationShip.bigId='"+(nUserId)+"') or (IMUser.id=IMRelationShip.bigId and "
        		"IMRelationShip.smallId='"+(nUserId)+"')) and IMRelationShip.status="+
        ...
        if(pResultSet)
        {
            while (pResultSet->Next())
            {
            	...
            	pUser->set_user_id(pResultSet->GetString("id"));
        ...
        strSql = "select IMUser.* from IMUser,IMRelationShip where (IMUser.id=IMRelationShip.smallId "
				"and IMRelationShip.bigId='"+(nUserId)+"' and (IMRelationShip.status=" +
...
void CUserModel::getUsersByNickOrPhone(string &sName,list<IM::BaseDefine::UserInfo> &lsUsers)
{
    ...
    if (pDBConn)
    {
        ...
        {
            //log(" no result sql:%s", strSql.c_str());
        }

        string strSql2 = "select * from IMGroup where type=" + int2string(IM::BaseDefine::GROUP_TYPE_NORMAL) + " and name='"+
        		sName + "'";
        ...
        if(pResultSet2)
		{
			...
			{
				IM::BaseDefine::UserInfo cUser;
				cUser.set_user_id(pResultSet2->GetString("id"));
				cUser.set_user_gender(0);
				cUser.set_user_nick_name(pResultSet2->GetString("name"));
				cUser.set_user_domain("");
				cUser.set_user_real_name(pResultSet2->GetString("name"));
				cUser.set_user_tel("");    //用户电话隐藏
				cUser.set_email("");
				cUser.set_avatar_url(pResultSet2->GetString("avatar"));
				cUser.set_sign_info("");

				cUser.set_department_id(0);
				cUser.set_status(0x11);
				cUser.set_fans_cnt(pResultSet2->GetInt("userCnt"));
				lsUsers.push_back(cUser);
			}
		}
        else
		{
			log(" no result sql:%s", strSql2.c_str());
		}
        delete pResultSet2;
...
bool CUserModel::getUserById(string nUserId, DBUserInfo_t &cUser)
{
    bool bRet = false;
    CDBManager* pDBManager = CDBManager::getInstance();
    CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_slave");
    if (pDBConn)
    {
        string strSql = "select * from IMUser where id='"+(nUserId)+"'";
        CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
        if(pResultSet)
        {
            while (pResultSet->Next())
            {
                cUser.nId = pResultSet->GetString("id");
...
bool CUserModel::updateUser(string user_id, IM::BaseDefine::UserInfo &cUser)
{
    bool bRet = false;
    CDBManager* pDBManager = CDBManager::getInstance();
    ...
    if (pDBConn)
    {
        ...
        string strSql = "update IMUser set `sex`=" + int2string(cUser.user_gender())+
				...
				int2string(nNow) + " where id='"+(user_id)+"'";
...
void CUserModel::clearUserCounter(string nUserId, string nPeerId, IM::BaseDefine::SessionType nSessionType)
{
    if(IM::BaseDefine::SessionType_IsValid(nSessionType))
    {
        ...
        if (pCacheConn)
        {
            ...
            {
                int nRet = pCacheConn->hdel("unread_" + (nUserId), (nPeerId));
                if(!nRet)
                {
                    log("hdel failed %s->%s", nPeerId.c_str(), nUserId.c_str());
                }
            }
            // Clear Group msg Counter
            else if(nSessionType == IM::BaseDefine::SESSION_TYPE_GROUP)
            {
                string strGroupKey = (nPeerId) + GROUP_TOTAL_MSG_COUNTER_REDIS_KEY_SUFFIX;
                map<string, string> mapGroupCount;
                bool bRet = pCacheConn->hgetAll(strGroupKey, mapGroupCount);
                if(bRet)
                {
                    string strUserKey = (nUserId) + "_" + (nPeerId) + GROUP_USER_MSG_COUNTER_REDIS_KEY_SUFFIX;
        ...
        log("invalid sessionType. userId=%s, fromId=%s, sessionType=%u", nUserId.c_str(), nPeerId.c_str(), nSessionType);
    }
}

void CUserModel::setCallReport(string nUserId, string nPeerId, IM::BaseDefine::ClientType nClientType)
{
    ...
    {
        log("invalid clienttype. userId=%s, peerId=%s, clientType=%u", nUserId.c_str(), nPeerId.c_str(), nClientType);
    }
}

void CUserModel::changeAvatar(string nUserId,string &avatar)
{
	bool rv = false;
	...
	if (db_conn) {
		...
		string str_sql = "update IMUser set `avatar`='" + avatar + "', `updated`=" + int2string(now) + " where id='"+(nUserId)+"'";
...
bool CUserModel::updateUserSignInfo(string user_id, const string& sign_info)
{
    ...
    if (db_conn) {
        uint32_t now = (uint32_t)time(NULL);
        string str_sql = "update IMUser set `sign_info`='" + sign_info + "', `updated`=" + int2string(now) + " where id='"+(user_id)+"'";
        rv = db_conn->ExecuteUpdate(str_sql.c_str());
        if(!rv) {
...
bool CUserModel::getUserSingInfo(string user_id, string* sign_info)
{
    bool rv = false;
    CDBManager* db_manager = CDBManager::getInstance();
    CDBConn* db_conn = db_manager->GetDBConn("teamtalk_slave");
    if (db_conn) {
        string str_sql = "select sign_info from IMUser where id='"+(user_id)+"'";
...
bool CUserModel::updatePushShield(string user_id, uint32_t shield_status) {
    bool rv = false;
    ...
    if (db_conn) {
        uint32_t now = (uint32_t)time(NULL);
        string str_sql = "update IMUser set `push_shield_status`="+ int2string(shield_status) + ", `updated`=" + int2string(now) + " where id='"+(user_id)+"'";
        rv = db_conn->ExecuteUpdate(str_sql.c_str());
        if(!rv) {
...
bool CUserModel::getPushShield(string user_id, uint32_t* shield_status) {
    bool rv = false;
    
    CDBManager* db_manager = CDBManager::getInstance();
    CDBConn* db_conn = db_manager->GetDBConn("teamtalk_slave");
    if (db_conn) {
        string str_sql = "select push_shield_status from IMUser where id='"+(user_id)+"'";
        CResultSet* result_set = db_conn->ExecuteQuery(str_sql.c_str());
        if(result_set) {
            if (result_set->Next()) {
...
bool CUserModel::updateFansCnt(string user_id, bool isIncrease)
{
    bool bRet = false;
    CDBManager* pDBManager = CDBManager::getInstance();
    CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_master");
    if (pDBConn)
    {
        ...
        string strSql;
        if(isIncrease){
        	strSql = "update IMUser set `fans_cnt`=fans_cnt+1, `updated`="+int2string(nNow) + " where id='"+(user_id)+"'";
        }else{
        	strSql = "update IMUser set `fans_cnt`=fans_cnt-1, `updated`="+int2string(nNow) + " where id='"+(user_id)+"'";
        }
